...
def test_default_strategy_fallback_policy_shape_routes_in_priority_order(
    routing_client: tuple[TestClient, dict[str, str], dict[str, str]],
) -> None:
    client, master_header, _api_key_header = routing_client
...
def test_default_strategy_intelligent_policy_shape_uses_axis_and_candidate_pricing(
    routing_client: tuple[TestClient, dict[str, str], dict[str, str]],
) -> None:
    client, master_header, _api_key_header = routing_client
    ...
    response = client.post(
        "/v1/routing-policies",
        json={
            "name": "Default strategy intelligent",
            "is_default": True,
            "default_strategy": {
                "type": "intelligent",
                "axis": "intelligence",
...
def test_default_strategy_can_update_existing_policy(
    routing_client: tuple[TestClient, dict[str, str], dict[str, str]],
) -> None:
    client, master_header, _api_key_header = routing_client
    policy = _create_policy(
    ...
    response = client.patch(
        ...
        json={
            ...
            "change_note": "switch to default_strategy policy shape",
        },
        headers=master_header,
    )
    ...
    assert revisions_response.status_code == 200
    revisions = revisions_response.json()
    assert revisions[0]["change_note"] == "switch to default_strategy policy shape"
    assert revisions[0]["default_strategy"]["type"] == "fallback"
    assert revisions[0]["default_strategy"]["providers"][0]["provider"] == "anthropic"


def test_routing_resolve_accepts_default_routing_sentinels(
    routing_client: tuple[TestClient, dict[str, str], dict[str, str]],
) -> None:
    client, master_header, _api_key_header = routing_client
    policy = _create_policy(
...
def test_chat_completions_accepts_default_routing_sentinels(
    routing_client: tuple[TestClient, dict[str, str], dict[str, str]],
) -> None:
    client, master_header, api_key_header = routing_client
    _create_policy(
...
def test_responses_endpoint_accepts_default_routing_sentinels(
    routing_client: tuple[TestClient, dict[str, str], dict[str, str]],
) -> None:
    client, master_header, api_key_header = routing_client
    _create_policy(
