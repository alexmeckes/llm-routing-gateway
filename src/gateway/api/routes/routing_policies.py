...
_DEFAULT_STRATEGY_TYPES = {"fallback", "intelligent", "weighted_score"}
_INTELLIGENT_AXES = {"cost", "performance", "intelligence"}
_AXIS_TIER_THRESHOLDS = {
...
def _config_from_default_strategy(
    ...
    *,
    ...
    if strategy_type not in _DEFAULT_STRATEGY_TYPES:
        supported = ", ".join(sorted(_DEFAULT_STRATEGY_TYPES))
    ...
    if axis not in _INTELLIGENT_AXES:
        supported = ", ".join(sorted(_INTELLIGENT_AXES))
    ...
    config["axis"] = axis
    config.setdefault("fallback_enabled", True)
    config.setdefault("tier_thresholds", _AXIS_TIER_THRESHOLDS[axis])
...
def _create_policy_shape(request: CreateRoutingPolicyRequest) -> tuple[str, dict[str, Any]]:
    if request.default_strategy is not None:
        return _config_from_default_strategy(request.default_strategy, base_config=request.config)
...
def _update_policy_shape(
    ...
    return _config_from_default_strategy(default_strategy, base_config=base_config)
